...
def test_register_user(api_client):
    ...
    data = {"email": "newuser@example.com", "password": "StrongPass123!", "first_name": "New"}
...
def test_login_user(api_client):
    # Авторизация
    get_user_model().objects.create_user(email="testlogin@example.com", password="TestLoginPass1!")
...
@pytest.mark.django_db
def test_set_telegram_id(api_client, user):
    """Проверка установки chat_id через POST-запрос."""
    # Изначально поле пустое
    assert user.telegram_chat_id is None
    api_client.force_authenticate(user=user)
    response = api_client.post("/api/v1/auth/set_telegram_id/", {"chat_id": 12345}, format="json")
    assert response.status_code == status.HTTP_200_OK
    assert "status" in response.data and response.data["status"] == "ok"
    # Перечитываем объект из базы, чтобы убедиться, что он сохранился
